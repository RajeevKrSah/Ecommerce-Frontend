...
export default function AdminColorsPage() {
  ...
  const loadColors = useCallback(async () => {
    try {
      ...
      const errorMessage = error?.message || 'Failed to load colors';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      addToast({ type: 'error', message: 'Color name is required' });
      return;
    }
    if (!formData.code.trim()) {
      addToast({ type: 'error', message: 'Color code is required' });
      return;
    }
    if (!/^#[0-9A-Fa-f]{6}$/.test(formData.hex_code)) {
      addToast({ type: 'error', message: 'Invalid hex code format' });
      return;
    }
    ...
    try {
      ...
      const errorMessage = error?.message || error?.response?.data?.message || 'Failed to save color';
      addToast({
        type: 'error',
        message: errorMessage,
      });
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      ...
      const errorMessage = error?.message || error?.response?.data?.message || 'Failed to delete color';
  ...
  const toggleColorStatus = async (color: Color) => {
    try {
      ...
      console.error('Failed to update color status:', error);
      const errorMessage = error?.message || error?.response?.data?.message || 'Failed to update color status';
      addToast({ type: 'error', message: errorMessage });
    }
  };
    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-red-900 mb-2">Failed to Load Colors</h3>
